...
import { LessonContent, Subject, ClassLevel, Chapter, MCQItem, ContentType, User } from '../types';
import { ArrowLeft, Clock, AlertTriangle, ExternalLink, CheckCircle, XCircle, Trophy, BookOpen, Play, Lock } from 'lucide-react';
...
interface Props {
  ...
  chapter: Chapter;
  loading: boolean;
  user?: User; // Made optional to be backward compatible but should be passed for MCQ
  onBack: () => void;
...
export const LessonView: React.FC<Props> = ({ 
  ...
  // New States for Flow
  const [viewMode, setViewMode] = useState<'QUIZ' | 'SUMMARY' | 'ANALYSIS'>('QUIZ');
  ...
  if ((content.type === 'MCQ_ANALYSIS' || content.type === 'MCQ_SIMPLE') && content.mcqData) {
      ...
      useEffect(() => {
          ...
          if (saved) {
              ...
              // We will load the exact state from resume handle
          } else {
      ...
      // --- SAVE PROGRESS LOGIC ---
      useEffect(() => {
          if (viewMode === 'QUIZ' && Object.keys(mcqState).length > 0) {
              const key = `nst_mcq_progress_${chapter.id}`;
      ...
      }, [mcqState, batchIndex, chapter.id, localMcqData, viewMode]);
      ...
      const handleSubmitBatch = () => {
          ...
          // Clean up progress storage as they submitted
          const key = `nst_mcq_progress_${chapter.id}`;
          localStorage.removeItem(key);
      ...
      const handleUnlockAnalysis = () => {
          if (!user || !onUpdateUser) {
              alert("User data missing. Cannot process transaction.");
              return;
          }
          
          const ANALYSIS_COST = 5; // As per prompt
          if (user.credits < ANALYSIS_COST) {
              alert(`Insufficient Credits! You need ${ANALYSIS_COST} coins to view analysis.`);
              return;
          }
          
          if (!window.confirm(`View detailed analysis for ${ANALYSIS_COST} coins?`)) return;
          ...
          setBatchIndex(0); // Start from beginning for analysis
      };
            ...
            <div className="flex flex-col h-full bg-slate-50 animate-in fade-in relative">
                             ...
                             <div key={idx} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm transition-all hover:shadow-md">
                                 ...
                                 <div className="space-y-2">
                                     {q.options.map((opt, oIdx) => {
                                         // NO IMMEDIATE FEEDBACK (COLOR) - ONLY SELECTION
                                         ...
                                         if (userAnswer === oIdx) {
                                            ...
                                            btnClass += "bg-white border-slate-200 hover:bg-slate-50 hover:border-blue-200 text-slate-700";
                                         }
                                         ...
                                         return (
                                             ...
                                             >
                                                 <span className="relative z-10 flex justify-between items-center">
                                                     {opt}
                                                     {userAnswer === oIdx && <CheckCircle size={16} className="text-white" />}
                                                 </span>
                                             </button>
                                         );
                                     })}
                 ...
                 <div className="p-4 bg-white border-t border-slate-200 sticky bottom-0 z-10 flex justify-center shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] gap-4">
                     {hasMore && (
                         ...
                         >
                             Next Page ({displayData.length - (batchIndex + 1) * BATCH_SIZE} left)
                         </button>
                     )}
                     
                     <button 
                         onClick={handleSubmitBatch}
                         className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-8 rounded-xl shadow-lg transition-all active:scale-95 flex items-center gap-2"
                     >
      ...
      if (viewMode === 'SUMMARY') {
        return (
            <div className="flex flex-col items-center justify-center h-full p-8 bg-slate-50 animate-in zoom-in">
                <div className="bg-white rounded-3xl p-8 max-w-sm w-full text-center shadow-2xl border border-slate-100">
                    ...
                    <h2 className="text-3xl font-black text-slate-800 mb-2">Quiz Submitted!</h2>
                    <p className="text-slate-500 mb-8 font-medium">
                        Your answers have been recorded. <br/>
                        Would you like to see the detailed analysis?
                    </p>
                    ...
                    <div className="space-y-4">
                        ...
                        >
                            <Trophy size={20} /> View Analysis (5 Coins)
                        </button>
                        
                        <button 
                            onClick={onBack}
                            className="w-full bg-slate-100 text-slate-600 font-bold py-4 rounded-xl hover:bg-slate-200 transition-all active:scale-95 flex items-center justify-center gap-2"
                        >
                            <ArrowLeft size={20} /> Exit & Go Back
                        </button>
                    </div>
                </div>
            </div>
        ...
        return (
            <div className="flex flex-col h-full bg-slate-50 animate-in fade-in relative">
                 ...
                 <div className="flex items-center justify-between p-4 bg-white border-b border-slate-200 sticky top-0 z-10 shadow-sm">
                     ...
                     <div className="text-right">
                         <h3 className="font-bold text-slate-800 text-sm">Detailed Analysis</h3>
                         <span className="text-xs font-bold text-green-600">Score: {score}/{displayData.length}</span>
                     </div>
                 </div>
                     ...
                     {currentBatchData.map((q, localIdx) => {
                         ...
                         return (
                             <div key={idx} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
                                 <h4 className="font-bold text-slate-800 mb-4 flex gap-3 leading-relaxed">
                                     <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs shrink-0 font-bold mt-0.5 ${isCorrect ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                                         {idx + 1}
                                     </span>
                                     {q.question}
                                 </h4>
                                 <div className="space-y-2">
                                     {q.options.map((opt, oIdx) => {
                                         ...
                                         } else if (userAnswer === oIdx && userAnswer !== q.correctAnswer) {
